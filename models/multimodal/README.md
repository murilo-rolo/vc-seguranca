...
- **Emotion Features**: Embeddings de 128 dims (penúltima camada do EmotionNet) — classificação binária violent/non_violent. Por frame, **todas** as faces detectadas são agregadas em um único embedding (mean/max, `--face_aggregation`); sem faces → embedding neutro
